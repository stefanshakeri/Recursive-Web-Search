...
def get_keywords(index: int, total: int, doi: str) -> str:
    ...
    try:
        # Clean the DOI link
        response = requests.get(doi, headers=HDRS, timeout=20)
        response.raise_for_status()
        html_doc = response.text
        soup = BeautifulSoup(html_doc, "html.parser")  # Use built-in parser instead of lxml
        ...
        print(f"[{index}/{total}] Error fetching DOI {doi}: {e}")
        return "Unknown"
    except Exception as e:
        print(f"[{index}/{total}] Error processing DOI {doi}: {e}")
        return "Unknown"
